...
# @Last Modified time: 14.10.2019

"""
This script demonstrates how a building can be generated by importing all data from excel.
An appropriate example file with some building data is imported from examplefiles/ExcelBuildingData_Sample.xlsx.

In the excel every room is listed by its own, via a self defined zoning algorithm these rooms are combined to zones.
...
-yellowed columns are necessary input to teaser -> don´t change column header, keep value names consistent.
-non yellowed columns may either not be used or be used for your zoning algorithm

Information about the required excel format:
#Documentation in progress!
-Under the cell ‚Usage type‘  you will see some cells that are blank but have their row filled.
It means the blank cell actually belongs to the Usage type above but in that specific row we filled the characteristics
of the window/wall of a different orientation of the same exact room. That means every row is either a new room or a
new orientation of that room. A room might have two outerwalls in two different orientation so for each outer wall,
a an extra row defining the respective orientiation is added
-The entries in the excel sheet must be consistent for python being able to convert it.
...
def import_data(Path=None, SheetNames=[0, 1, 2, 3, 4]):
    ...
    if type(SheetNames) == list:
        ...
        Data = pd.read_excel(io=Path, sheet_name=SheetNames, header=0, index_col=0)
...
# -------------------------------------------------------------
# The zoning should return the imported dataset with an additional column called "Zone" which inhibits the information
# to which zone the respective room shall be part of.
def zoning_example(Data):
    '''
    This is an example on how the rooms of a building could be aggregated to zones.

    In this example the UsageType has to be empty in the case that the respective line does not represent another
    room but a different orientated wall or window belonging to a room that is already declared once in the excel file.
    '''

    # account all outer walls not adjacent to the ambient to the entity "inner wall"
    # !right now the wall construction of the added wall is not respected, the same wall construction as regular
    # inner wall is set
    ...
    for i, row in Data.iterrows():
        if row["NetArea[m²]"] == 0 and not pd.isna(row["UsageType"]):
            warnings.warn(
                "In line %s the net area is zero, marking an second wall or window element for the respective room, "
    ...
    for index, cluster in _groups:
        ...
        if count != 1:
            warnings.warn(
                ...
                "Every row with an empty slot in the column UsageType, marks another direction of an outer wall and/or"
                "window entity of the same room.\n"
                "The connection of the same room is realised by an RoomIdentifier equal to the respective "
                "BelongsToIdentifier. \n Cluster = %s" %cluster)

    # name usage types after usage types available in the json
    usage_to_json_usage = {"IsolationRoom": "Bed room",
...
def import_building_from_excel(prj, building_name, construction_age, excel_name, SheetNames):
    ...
    def warn_constructiontype(element):
        if element.construction_type == None:
            warnings.warn(
                "In Zone \"%s\" the %s construction \"%s\" could not be loaded from the "
                "TypeBuildingElements.json, an error will occur due to missing data for calculation."
                "Check for spelling and the correct combination of building age and construction type."
                "Here is the list of faulty entries:\n%s"
                "\nThese entries can easily be found checking the stated index in the produced "
                "ZonedInput.xlsx"
    ...
    # 1. "use_set_back" needs hard coding at aixlib.py in the init; defines if the in the useconditions stated
    #   heating_time with the respective set_back_temp should be applied. use_set_back = false -> all hours of the day
    ...
    for name, Zone in Zones:
        ...
        # load UsageOperationTime, Lighting, RoomClimate and InternalGains from the "UseCondition.json"
        ...
        for name, group in Grouped:
            ...
            if not isinstance(group["OuterWallOrientation[°]"].iloc[0], str):
                ...
                warnings.warn(
                    ...
                    "hence this building element is not added. Here is the list of faulty entries:\n%s"
                    "\nThese entries can easily be found checking the stated index in the produced ZonedInput.xlsx"
            ...
            if group["NetArea[m²]"].sum() != 0:  # to avoid devision by 0
                if group["IsRooftop"].iloc[0] == 1:
                    ...
                    ceiling.name = "ceiling" + \
                                   str(group["CeilingConstruction"].iloc[0])
                    ceiling.area = group[
                                       "NetArea[m²]"].sum() / 2  # only half of the ceiling belongs to a story,
        ...
        for name, group in Grouped:
            if group["InnerWallArea[m²]"].sum() != 0:  # to avoid devision by 0
                ...
                in_wall.area = group[
                                   "InnerWallArea[m²]"].sum() / 2  # only half of the wall belongs to each room,
        ...
        if _i == 0:
            warnings.warn(
                "The zone %s could not be found in your ahu_dict. Hence, no AHU flow is defined. The default value is "
...
if __name__ == '__main__':
    ...
    prj, Data = import_building_from_excel(prj, "ExampleImport", 2000, PathToExcel,
                                           SheetNames=["ImportSheet1"])
    ...
    # if wished, save the current python script to the resultsfolder to track the used parameters and reproduce results
