...
class TabulaTH(Residential):
    """Archetype Residential Building according

    Subclass from Residential archetpye class to represent a Tabula building
    from "https://github.com/RWTH-EBC/TabulaTEASER".
    The Tabula module contains a singlezone building. It makes
    number_of_floors and height_of_floors mandatory parameters.
    Additional information can be passed to the archetype
    (e.g. floor layout and number of neighbors).
    ...
    construction_type : str
        Construction type of used wall constructions default is
        "tabula_standart_TH")
            tabula_standart_TH:
            tabula_refurbished_TH:
            tabula_advanced_refurbished_TH:
    ...
    dict_bak_TH: dictionary
        dictionary which contains factors to calculate the est factor
    """
    ...
    def __init__(
        ...
        """Constructor of TabulaBuilding
        """

        super(TabulaTH, self).__init__(
            parent,
            name,
    ...
    def calc_est_factor(self, input_data):
        """Calculates est_factor

        Calculates the est_factor with a given construction year and a mode of
        calculation. The construction year is iterated over a dictionary and
        checked if its beetween a building age group.
        """

        for key, value in self.dict_bak_TH.items():
            if key[1] >= input_data[0] >= key[0]:
                if input_data[2] == "S/F":
                    return(input_data[1] * value[0])
                elif input_data[2] == "S/(F*n2)":
                    return(input_data[1] * value[1])
                break

    def generate_archetype(self):
        """Generates a Tabula building.
    ...
    def construction_type(self, value):
        if value is not None:
            if value == "tabula_standard_TH" or \
               value == "tabula_refurbished_TH" or \
               value == "tabula_advanced_refurbished_TH":
                self._construction_type = value
            else:
                raise ValueError("Construction_type has to be" +
                                 "tabula_standard_TH, " +
                                 "tabula_refurbished_TH or" +
                                 "tabula_advanced_refurbished_TH")
        else:
            self._construction_type = "tabula_standard_TH"
