...
class GroceryStore(Office):
    ...
    The grocery store module contains a multi zone building which is based
    on an office building with additional retail, rooms and kitchen zones.
    ...
    areas. Due to the size of the grocery store (small/medium/big) the ratio
    of the net leased area differs:

    #. Retail with cooling (74-77% of net leased area)
    #. Storage (14-18% of net leased area)
    #. Rooms (6-8% of net leased area)
    #. Kitchen (1-2% of net leased area)
    ...
    def __init__(self,
        ...
        """Constructor of GroceryStore

        Adds a additional zones "Retail", "Rooms" and "Kitchen"

        """

        super(GroceryStore, self).__init__(parent,
                                           name,
        ...
        if net_leased_area < 1152 * 0.89:
            ...
            self.zone_area_factors["Retail with cooling"] = \
                [0.74, "Retail with cooling"]
            self.zone_area_factors["Storage"] = \
                [0.18, "Stock, technical equipment, archives"]
            self.zone_area_factors["Rooms"] = \
                [0.06, "Further common rooms"]
            self.zone_area_factors["Kitchen"] = \
                [0.02, "Kitchen - preparations, storage"]

            self.height_of_floors = 4.0

            self.est_exponent_wall = 1.0
            self.est_factor_wall_area = 0.53
            self.est_factor_win_area = 0.068
            self.est_exponent_win = 1

        # medium grocery store
        elif net_leased_area < 2520 * 0.89:
            self.zone_area_factors["Retail"] = \
                [0.77, "Retail with cooling"]
            self.zone_area_factors["Storage"] = \
                [0.14, "Stock, technical equipment, archives"]
            self.zone_area_factors["Rooms"] = \
                [0.08, "Further common rooms"]
            self.zone_area_factors["Kitchen"] = \
                [0.01, "Kitchen - preparations, storage"]

            self.height_of_floors = 4.5

            self.est_exponent_wall = 1.0
            self.est_factor_wall_area = 0.433
            self.est_factor_win_area = 0.015
            self.est_exponent_win = 1

        # big grocery store
        else:
            self.zone_area_factors["Retail"] = \
                [0.75, "Retail with cooling"]
            self.zone_area_factors["Storage"] = \
                [0.16, "Stock, technical equipment, archives"]
            self.zone_area_factors["Rooms"] = \
                [0.08, "Further common rooms"]
            self.zone_area_factors["Kitchen"] = \
                [0.02, "Kitchen - preparations, storage"]

            self.height_of_floors = 5.4

            self.est_exponent_wall = 1.0
            self.est_factor_wall_area = 0.309
            self.est_factor_win_area = 0.0096
            self.est_exponent_win = 1
