...
class Wall(BuildingElement):
    ...
    def calc_equivalent_res(self, t_bt=7):
        ...
        re11 = np.cosh(np.sqrt(0.5 * omega * r_layer * c_layer)) * np.cos(
            np.sqrt(0.5 * omega * r_layer * c_layer)
        )
        im11 = np.sinh(np.sqrt(0.5 * omega * r_layer * c_layer)) * np.sin(
            np.sqrt(0.5 * omega * r_layer * c_layer)
        )
        re12 = (
            r_layer
            * np.sqrt(1 / (2 * omega * r_layer * c_layer))
            * (
        ...
        self.r1 = (
            (1 / self.area)
            * ((new_mat[3][3] - 1) * new_mat[0][2] + new_mat[2][3] * new_mat[0][3])
            / ((new_mat[3][3] - 1) ** 2 + new_mat[2][3] ** 2)
        )
        self.r2 = (
            (1 / self.area)
            * ((new_mat[0][0] - 1) * new_mat[0][2] + new_mat[0][1] * new_mat[0][3])
        ...
        self.c1 = (
            ...
            * ((new_mat[3][3] - 1) ** 2 + (new_mat[2][3]) ** 2)
            / (
                omega
                * (new_mat[0][2] * new_mat[2][3] - (new_mat[3][3] - 1) * new_mat[0][3])
            )
        )
        self.c2 = (
            self.area
            * ((new_mat[0][0] - 1) ** 2 + (new_mat[0][1]) ** 2)
            / (
                omega
                * (new_mat[0][2] * new_mat[0][1] - (new_mat[0][0] - 1) * new_mat[0][3])
            )
        )
        ...
        self.c1_korr = (1 / (omega * self.r1)) * (
            ...
            / (new_mat[3][3] * new_mat[0][3] - new_mat[0][2] * new_mat[2][3])
        )

        if (
            type(self).__name__ == "OuterWall"
    ...
    def retrofit_wall(self, year_of_retrofit, material=None):
        ...
        if year_of_retrofit < 1977:
            ...
            warnings.warn(
                "You are using a year of retrofit not supported\
        ...
        if type(self).__name__ == "OuterWall":
            ...
            elif 2014 <= year_of_retrofit <= 2015:
                self.insulate_wall(material)
                calc_u = 0.24 * self.area
        ...
        if self.ua_value < calc_u:
            ...
            for count_layer in self.layer[:-1]:
                r_conduc += count_layer.thickness / count_layer.material.thermal_conduc

                self.layer[-1].thickness = (
                    (
                        (1 - calc_u * self.r_inner_comb - calc_u * self.r_outer_comb)
                        / calc_u
                    )
                    * self.area
                    - r_conduc
