...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(columns=['Scenario', 'Building type', 'Num Zones','Heat demand total','Net area','Construction year'],index=range(10))
    ...
    for idx, path in enumerate(pathlist):
        ...
        df.loc[idx] = {'Scenario': scenario, 'Building type': building_type, 'Num Zones': num_zones,
                       'Heat demand total': heat_demand_total, 'Net area': net_area, 'Construction year': year_of_construction}

    df["Specific heat demand"] = df['Heat demand total']/df['Net area']
    df.to_pickle(basepath.joinpath('dataframe_heat_demand_total.pkl'))
